...
2.Show a regression summary for both the kitchen sink and your engineered regression. #what is a kitchen sink here? AD: I think it's just an expression meaning everything/random assortment of features
...
#ageTest <- housingsubsidy %>%
 # filter(tookSubsidy == "yes")

#  qplot(ageTest$lastContact, geom="histogram") +
 #   stat_bin(bins = 20)
...
reg0 <- glm(tookSubsidyNum ~ .,
                  data=housingsubsidyTrain %>% 
                    dplyr::select(-X, -tookSubsidy, -industry, -degreeGroup, -employStatus,-contactedBefore), #delete all the NAs?
...
#with variables based on exp() of coefficients
reg4 <- glm(tookSubsidyNum ~ .,
                  data=housingsubsidyTrain %>% 
                    dplyr::select(tookSubsidyNum, marital, month, prevMktgOutcome, unemploy_rate, cons.price.idx, employStatus, ageGroup, degreeGroup, industry, lastContact, mortgage),
                  family="binomial" (link="logit"))
...
cvFit <- train(tookSubsidy ~ ., data = housingsubsidy %>% 
                                   dplyr::select(
                                   -tookSubsidyNum), 
...
cost_benefit_table <-
   testProbs %>%
      ...
      summarize(True_Negative = sum(n[predOutcome==0 & Outcome==0]),
       ...
       mutate(Revenue =
               case_when(Variable == "True_Negative"  ~ 0,  #AD: not sure about this section, can you double check? #BC: I think we reaaly need a meeting to discuss this, I think the premium only applies to those who accept the credits
                         Variable == "True_Positive"  ~ ((10000 - 7850) * (Count * .25)) + (-12850 * (Count * .75)), #AD: we should be using 0.75 - the book uses 0.5 for both because they expect half to churn while we expect 1/4 to accept subsidy
                         Variable == "False_Negative" ~ (-10000) * Count,
                         Variable == "False_Positive" ~ (10000-7850)*Count)) %>%
...
iterateThresholds <- function(data) {
  ...
  this_prediction <-
     ...
     mutate(Revenue =
               ...
               ifelse(Variable == "True_Positive",((10000 - 7850) * Count),
               ifelse(Variable == "False_Negative", (-10000) * Count,
...
#calculate revenue at each threshold    #AD: I think the problem is here
#whichThreshold <- 
 # whichThreshold %>%
...
#                           Variable == "Count_TP"  ~ ((10000 - 7850) * (Count * .25)) + (-12850 * #(Count * .75)),
#                           Variable == "Count_FN"  ~ (-10000) * Count,
#                           Variable == "Count_FP"  ~ (10000-7850)*Count))
...
whichThreshold_revenue <- 
  whichThreshold %>% 
    mutate(actualNonreply = ifelse(Variable == "Count_TP", (Count * .75),
                         ifelse(Variable == "Count_FN", Count, 0))) %>% 
    group_by(Threshold) %>% 
    summarize(Revenue = sum(Revenue),
              Actual_Nonreply_Rate = sum(actualNonreply) / sum(Count),
              Actual_Nonreply_Revenue_Loss =  sum(actualNonreply * 10000),
              Revenue_Next_Period = Revenue - Actual_Nonreply_Revenue_Loss) 
...
```{r Create a table}
threshold_table <- whichThreshold_revenue[c(1,50,), c("Threshold", "Revenue", "Revenue_Next_Period")]
threshold_table %>%
  group_by(Thresholdn) %>% 
  kable(caption = "Total_Revenue and Total_Count_of_Credits") %>%
    kable_styling("striped", full_width = F) 
#not finishes, need nopredictive data and best threshold data
```
