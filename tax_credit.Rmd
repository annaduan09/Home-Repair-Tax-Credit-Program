...
housingsubsidy %>%
  dplyr::select(y,age,job, marital, education,mortgage, contact, month, day_of_week, spent_on_repairs, unemploy_rate, poutcome, previous, campaign, pdays, taxLien, taxbill_in_phl) %>%
...
housingsubsidy %>%
    dplyr::select(y,age,spent_on_repairs,unemploy_rate,campaign) %>%
    gather(Variable, value, -y) %>%
    ggplot() + 
