...
reg2 <- glm(tookSubsidyNum ~ .,
                  data=housingsubsidyTrain %>% 
                    dplyr::select(-age, -job, -education, -pdays, -previous, -X, -tookSubsidy),
                  family="binomial" (link="logit"))
...
testProbs <- data.frame(Outcome = as.factor(housingsubsidyTest$tookSubsidyNum),
                        Probs = predict(reg0, housingsubsidyTest, type= "response"))
testProbs <- 
  testProbs %>%
      ...
      summarize(True_Negative = sum(n[predOutcome==0 & Outcome==0]),
       ...
       mutate(Revenue =
               case_when(Variable == "True_Negative"  ~ 0,  #AD: not sure about this section, can you double check? #BC: I think we reaaly need a meeting to discuss this, I think the premium only applies to those who accept the credits
                         Variable == "True_Positive"  ~ 0.25*Count*(56000+10000)-(Count*2850+0.25*Count*5000),
                         Variable == "False_Negative" ~ Count*(56000+10000)-(5000 * Count),
...
#calculate revenue at each threshold
whichThreshold <- 
  whichThreshold %>%
    ...
    mutate(Revenue =
                 case_when(Variable == "Count_TN"  ~ 0,
                           Variable == "Count_TP"  ~ 0.25*Count*(56000+10000)-(Count*2850+0.25*Count*5000),
                           Variable == "Count_FN"  ~ Count*(56000+10000)-(5000 * Count),
...
whichThreshold_revenue <- 
  whichThreshold %>% 
    mutate(actualChurn = ifelse(Variable == "Count_TP", (Count * .75),
                         ifelse(Variable == "Count_FN", Count, 0))) %>% 
    group_by(Threshold) %>% 
    summarize(Revenue = sum(Revenue),
              Actual_Churn_Rate = sum(actualChurn) / sum(Count),
              Actual_Churn_Revenue_Loss =  sum(actualChurn * 66000), #NOT SURE
