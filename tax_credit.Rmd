...
reg1 <- glm(tookSubsidyNum ~ .,
                  data=housingsubsidyTrain %>% 
                    dplyr::select(-employStatus, -ageGroup, -degreeGroup, -industry, -lastContact, -contactedBefore),
                  family="binomial" (link="logit"))



#with engineereed features
reg2 <- glm(tookSubsidyNum ~ .,
                  data=housingsubsidyTrain %>% 
                    dplyr::select(-age, -job, -education, -pdays, -previous),
                  family="binomial" (link="logit"))
...
ggplot(testProbs, aes(x = Probs, fill = as.factor(Outcome))) +     #AD: This doesn't look right..
  geom_density() +
  facet_grid(Outcome ~ .) +
...
#area under curve 
pROC::auc(testProbs$Outcome, testProbs$Probs) #not good, overfit
```
...
cost_benefit_table <-
   testProbs %>%
      ...
      summarize(True_Negative = sum(n[predOutcome==0 & Outcome==0]),
       ...
       mutate(Revenue =
               case_when(Variable == "True_Negative"  ~ Count * 65000,  #AD: not sure about this section, can you double check? 
                         Variable == "True_Positive"  ~ ((65000 - 7850) * (Count * .25)) + 
                                                        (-72850 * (Count * .25)),
                         Variable == "False_Negative" ~ (-65000) * Count,
                         Variable == "False_Positive" ~ (65000 - 7850) * Count)) %>%
...
#calculate revenue at each threeshold
whichThreshold <- 
  whichThreshold %>%
    dplyr::select(starts_with("Count"), Threshold) %>%
    gather(Variable, Count, -Threshold) %>%
    mutate(Revenue =
                 case_when(Variable == "Count_TN"  ~ Count * 65000,
                           Variable == "Count_TP"  ~ ((65000 - 7850) * (Count * .25)) + 
                                                          (-72850 * (Count * .25)),
                           Variable == "Count_FN"  ~ (-65000) * Count,
                           Variable == "Count_FP"  ~ (65000 - 7850) * Count))
