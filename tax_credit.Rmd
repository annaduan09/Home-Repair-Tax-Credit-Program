      ...
      summarize(True_Negative = sum(n[predOutcome==0 & Outcome==0]),
       ...
       mutate(Revenue =
               case_when(Variable == "True_Negative"  ~ 0,  
                         Variable == "True_Positive"  ~ ((10000 - 7850) * (Count * .25)) + (-2850 * (Count * .75)), #AD: we should be using 0.75 - the book uses 0.5 for both because they expect half to churn while we expect 1/4 to accept subsidy #BC: YEAH that makes sense! I think I probably made a typo here. For those 75%, I think HCD only lose the allocation money, the housing value remains unchanged since the owner chooses not to take the credits to refine his/her home.
...
whichThreshold_revenue <- 
  whichThreshold %>% 
    mutate(actualcreditno = ifelse(Variable == "True_Positive", (Count * .75), ifelse(Variable == "False_Positive", Count, 0)),
           actualcredityes = ifelse(Variable == "True_Positive", (Count * .25), ifelse(Variable == "False_Negative", Count, 0))) %>%  #0 OUT FN here
    group_by(Threshold) %>% 
    summarize(Revenue = sum(Revenue),
              sumcredityes = sum(actualcredityes)*5000,
...
```{r Create a table}
#0.91
threshold_table <- whichThreshold_revenue[c(50,91), c("Threshold", "Revenue", "sumcredityes")]
threshold_table %>%
  group_by(Threshold) %>%
